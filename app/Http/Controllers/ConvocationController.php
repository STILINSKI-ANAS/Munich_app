...
class ConvocationController extends Controller
{
    ...
    {
        return view('user.convocation.form');
    }
...
//            dd($payment);
            ...
            if ($payment && $payment->verified) {
                $convocation = Convocation::firstOrCreate(
                    ...
                    ['exam_id' => $registration->exam_id, 'payment_id' => $payment->id]
                );

                return $this->generateConvocationPDF($convocation);
            } else {
                return redirect()->back()->with('error', 'Le paiement n\'est pas encore validé.');
            }
        }

        return redirect()->back()->with('error', 'Invalid registration reference or payment reference!');
    }
